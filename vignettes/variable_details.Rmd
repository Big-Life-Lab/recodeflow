...
The dataframe **var_details** contains the instructions for function `rec_with_table` on how to transform the variables listed in the dataframe `var_sheet`.
...
Here we explain the columns in the dataframe `var_details`. Many of these columns are required for `rec_with_table()` to work. We will use the `sex` variable to illustrate how each column is specified:
...
2. **dummyVariable:** captures each category of a categorical variable. For continuous variables, this column is not applicable (set to `NA`).  The name of a dummy variable consists of the final variable name, the number of categories in the variable, and the category level for each category. Note that this column is not necessary for `rec_with_table()`.

```{r, echo=FALSE, warning=FALSE}
kable(var_details[c(7:11), c(1:2)])
```

3. **toType:** the type of the final transformed variable. In this column, a transformed variable that is categorical will be specified as `cat`; while a transformed variable that is continuous will be specified as `cont`.

```{r, echo=FALSE, warning=FALSE}
kable(var_details[c(7:11), c(1:3)])
```

4. **databaseStart:** the datasets that contain the variable of interest, separated by commas.

```{r, echo=FALSE, warning=FALSE}
kable(var_details[c(7:11), c(1:4)])
```

5. **variableStart:** the original names of the variables as they are listed in the original datasets, separated by commas. If the variable name in a particular dataset is different from the transformed variable name, write out the dataset name, add two colons, and write out the original variable name for that dataset (e.g., tester1::ascites). If the variable name in a particular dataset is the same as the transformed variable name, write the variable name in square brackets. Note: this only needs to be written out **once**.

```{r, echo=FALSE, warning=FALSE}
kable(var_details[c(7:11), c(1:5)])
```

6. **fromType:** the variable type as indicated in datasets. As indicated in the **toType** column, categorical variables are denoted as `cat` and continuous variables are denoted as `cont`.

```{r, echo=FALSE, warning=FALSE}
kable(var_details[c(7:11), c(1:6)])
```

7. **recTo:** the category of the categorical value your transforming to. For the not applicable category, write `NA::a` and for missing and else categories, write `NA::b`. For continuous variables that are not transformed in type, you would write in this column `copy` so that the function copies the values without any transformations.
...
12. **recFrom:** the range of values for a particular category in a variable as indicated in the original data. See data documentation and use the smallest and large values as your range to capture all values across the datasets. 
...
14. **variableStartShortLabel:** short form label describing the original variable.

```{r, echo=FALSE, warning=FALSE}
kable(var_details[c(7:11), c(1:14)])
```

15. **variableStartLabel:** more detailed label describing the original variable. This label should be identical to what is in the data documentation.
