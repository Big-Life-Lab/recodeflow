...
Specifically, the function `rec_with-table` recodes variables listed in column `variable_details$variableStart` to the variables listed in `variable_details$variable`. 
...
Each row in `variable_details` lists the instructions to recode a single category of a variable.
...
The number of rows required for a recoded variable depends on the:
