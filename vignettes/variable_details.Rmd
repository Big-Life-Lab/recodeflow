...
4. **databaseStart:** the surveys that contain the variable of interest, separated by commas.

```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(198:202), c(1:4)])
```

5. **variableStart:** the original names of the variables as they are listed in each respective survey cycle, separated by commas. If the variable name in a particular survey is different from the transformed variable name, write out the survey identifier, add two colons, and write out the original variable name for that cycle. If the variable name in a particular survey is the same as the transformed variable name, the variable name is written out surrounded by square brackets. Note: this only needs to be written out **once**.

```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(198:202), c(1:5)])
```

6. **fromType:** the variable type as indicated in surveys. As indicated in the **toType** column, categorical variables are denoted as `cat` and continuous variables are denoted as `cont`.
...
10. **catLabelLong:** more detailed label describing the category of a particular variable. This label should be identical to what is shown in the survey data documentation, unless you are creating derived variables and would like to create your own label for it.
...
12. **recFrom:** the range of values for a particular category in a variable as indicated in the survey. See data documentation for each survey cycle and use the smallest and large values as your range to capture all values between the survey years. 
...
13. **catStartLabel:** label describing each category. This label should be identical to what is shown in the CCHS data documentation. For the missing row, each missing category is described along with their coded values. You can import labels from a survey's DDI files using `bllflow` helper functions. See [bllflow documentation](http://bllflow.projectbiglife.ca).
...
15. **variableStartLabel:** more detailed label describing the variable. This label should be identical to what is shown in the survey data documentation.

```{r, echo=FALSE, warning=FALSE}
kable(variable_details[c(198:202), c(1:15)])
```

16. **notes:** any relevant notes to inform the user running the `recode-with-table` function. Things to include here would be changes in wording between surveys, missing/changes in categories, and changes in variable type between survey cycles.
