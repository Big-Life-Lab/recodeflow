...
2. **toType:** the data type of the final transformed variable. If the transformed variable is continuous fill in as `cont`; if the transformed variable that is categorical fill in as `cat` .

```{r, echo=FALSE, warning=FALSE}
kable(var_details[c(7:11), c(1:2)])
```

3. **fromType:** the variable type as indicated in datasets. As indicated in the **toType** column, categorical variables are denoted as `cat` and continuous variables are denoted as `cont`.
...
9. **recFrom:** the range of values for a particular category in a variable as indicated in the original data. See data documentation and use the smallest and large values as your range to capture all values across the datasets. 
...
10. **catLabel:** short form label describing the category of a particular variable.

```{r, echo=FALSE, warning=FALSE}
kable(var_details[c(7:11), c(1:10)])
```

11. **catLabelLong:** more detailed label describing the category of a particular variable. This label should be identical to what is recorded in the original data's documentation (e.g., the exact survey question). For derived variables, you can create your own long label.
