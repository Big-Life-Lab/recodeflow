...
Certain derived variables may need information from a table to complete their derivation. For example, imagine a derived variable that contains the average sodium consumption for an individual specifc to the individual's age and sex. It makes sense to put this information in a table that maps the individual's age and sex to their sodium consumption like below,
...
with the function for the derived variable shown below,
...
The reference_table argument expects a data frame.

Including a table in the start variables for a derived variable can be done using the syntax, `tables::<table_name>` where table_name contains the name of the table. For the `sodium_consumption` derived variable, the entry in the variables sheet would be,
