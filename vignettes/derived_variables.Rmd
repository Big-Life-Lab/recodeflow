...
Add the derived variable to the `var_sheet`. You'll use the same nomenclature as any other variable ([`variables_sheet`](../reference/variables.html)).

Add the derived variable to the variable_details. See the article [`variable_details`](../reference/variables.html) for nomenclature rules. 


### 3. Transform the derived variable

Use the function `rec_with_table` to transform your derived function. 
...
2) Transform the underlying variables (`chol` and `bili`) and the derived variable (`example_der`).
```{r, warning=FALSE}
