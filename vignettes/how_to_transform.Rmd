...
Using dataset tester1, we'll transform the variable `sex` into a harmonized `sex` variable. The harmonized `sex` variable has the values: 0 for males and 1 for females. 

1) Transform the `sex` variable in tester1.
```{r}
...
We'll transform and combine the variable `sex` for our two datasets. 
...
### Option 1: transform category `agegrp` variable into a common variable for only datasets with the same category responses

Transform the `agegrp` variable into a common variable only in datasets were the categories are the same. If the categories are different between datasets, separate columns will be created. 
...
1) Transform `agegrp5` in tester1 and transform `agegrp10` in tester2.
```{r, warning=FALSE}
...
Transform categorical variable `agegrp` into a single harmonized continuous
variable `age_cont`. 
...
1) Transform variable `agegrp` in tester1 and `agegrp` in tester2 to the harmonized continuous variable `age_cont`. 
```{r, warning=FALSE}
...
1) Transform variable `agegrp` in tester1 into `agegrp10`. Transform variable `agegrp` in tester2 into `agegrp10`.
```{r, warning=FALSE}
...
1) Transform `age` and `sex` in dataset tester1 and tester2
```{r, warning=FALSE}
...
You can check if labels have been added to your transformed dataset by using `get_label()`.
...
## Example 5. How transform all variables in variables_sheet {#example5}

All the variables listed in `variables_sheet` worksheet can be transformed with `rec_with_table()`. 

In this example, all variables specified in the `variables_sheet` worksheet will be transformed and combined for the datasets tester1 and tester2.

```{r, echo=FALSE}
options(htmlwidgets.TOJSON_ARGS = list(na = "string"))
```

1) Transform all variables variables_sheet, for dataset tester1
```{r, warning=FALSE}
#Transform all variables listed in variables_sheet, for dataset tester1
...
#Transform all variables listed in variables_sheet, for dataset tester2
...
2) Combine transformed datasets
```{r, warning=FALSE}
combined_dataset <- bind_rows(transformed1, transformed2)
```

3) Set labels for the combined transformed dataset
```{r}
...
## Example 6: How to know the a data origin in combined datasets {#example6}
...
1) Transform variables `age` and `sex` and attach dataset name for tester1 and tester2.
```{r, warning=FALSE}
age_sex_1 <- rec_with_table(data = tester1,
                            variables = c("age", "sex"), 
                            variable_details = variable_details,
...
Descriptions of derived functions are in the article [derived functions](../reference/derived_variables.html)


To transform a derived variable, you must: 

- create a customized function,
- defined the derived variable on the worksheets `variables_sheet` and `variable_details`,
- transform the variables that make up the derived variable. 

Our example derived variable `example_der` equals `chol` times `bili`.

1) Transform the underlying variables: `chol` and `bili` and the derived variable `example_der` for tester1 and tester2.
```{r, warning=FALSE}
