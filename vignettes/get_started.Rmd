...
Use `rec_with_table()` to transform the variables of a dataset.

## Example 1. Transform a single variable from a single database

In this example, the `status` variable in the test1 cycle is transformed into a
harmonized `status` variable that is consistent all datasets.

```{r}
status <- rec_with_table(tester, "status",
  log = TRUE,
...
are included in `variables` and `variable_details`, both included in
_recodeflow_. There are vignettes that further describe `variables` and
`variable_details`, including how to add or customize transformed variables.

## Which variables are transformed? How "good" is the transformation?

1. Consistent and unchanged across all data cycles. These variables have no
   changes to the wording of the question that was asked, the response values
   and who was asked the question (same inclusion criteria and skip pattern).
...
3. Either not collected in all cycles or have major changes between cycles.
   These variable may be included in _recodeflow_ with a note that says which
...
5. Not consistent between cycles. Variable that are inconsistent or not asked in
   most cycles are not included in _recodeflow_. In the future, we hope to include
   these variables with an marker that they've been reviewed but rejected from
   inclusion in _recodeflow_.
...
```{r}
warning(
  "recodeflow includes ", length(unique(variable_details$notes)),
...
If you've been watching, you may have noticed that _recodeflow_ recodes the
values in factor or categorical variables for 6 ("not applicable"), 7
("don’t know"), and 8 ("refusal"), 9 ("not stated") into `tagged_NA` values
NA(a) ("not applicable") and NA (b) ("missing"). The reason being is that R and
other open languages typically support only one NA.
...
## Example 3. Transform a single variable from multiple datasets

This example shows how you can transform and combine a variable across multiple
survey cycles. The sex variable in tester1 (`sex_a`) and tester2
(`sex_b`) is transformed a common variable (`sex`) and combined into a
single dataset. See the [variable_details vignette](variable_details.html) for
more information.

```{r, warning = FALSE}
sex_1 <- rec_with_table(tester1, "sex_a", log = TRUE)
head(sex_1)

sex_2 <- rec_with_table(tester2, "sex_b", log = TRUE)
tail(sex_2)
```
...
There are many variables in a survey cycle that can changes in categories
between cycles in a way that the variable cannot preserve all categories for all
CCHS cycles. An example is the categorical variable for `age`. _recodeflow_
offers three options to facilitate the use of these variables with inconsistent
categories across cycles.
...
transformed from a dataset. In this example, the age and sex variables from
the tester1 and tester2 datasets will be transformed and labeled using
`rec_with_table()`. They will then be combined into a single dataset and labeled
using `set_data_labels()`.

```{r, warning=FALSE}
age_sex_1 <- rec_with_table(tester1, c("DHHGAGE_cont", "DHH_SEX"),
  var_labels = c(DHHGAGE_cont = "Age", DHH_SEX = "Sex")
)

get_label(age_sex_1)

age_sex_2 <- rec_with_table(tester1, c("DHHGAGE_cont", "DHH_SEX"),
  var_labels = c(DHHGAGE_cont = "Age", DHH_SEX = "Sex")
)
...
combined, and labeled for the tester1 and tester2 datasets.
...
```{r, warning=FALSE}
transformed1 <- rec_with_table(data = tester1, notes = FALSE)

transformed2 <- rec_with_table(data = tester2, notes = FALSE)

combined_dataset <- bind_rows(transformed1, transformed2)
labeled_combined_cchs <- set_data_labels(
  data_to_label = combined_dataset,
  variable_details = variable_details, variables_sheet = variables
)
```
