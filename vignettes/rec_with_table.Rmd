...
`variables_sheet` worksheet or `rec_with_table()` call.
...
- `variables` Either a string of variables to recode or a dataframe containing the variables (e.g., `variables_sheet`)
...
#To recode all variables listed on the variable_sheet
...
- `var_labels` is set to NULL. This argument can be used to add labels to a
  ...
  `variables_sheet` worksheet. The format to add labels to a subset of variables is
  `c(variable = "Label")`.
