...
- **Enable reproducible, standardized metadata**: Use established conventions from `` `haven` `` and `` `labelled` `` so data analysts can apply labels consistently in R.

- **Facilitate collaboration**: Store metadata in human-readable CSV files (`variables` and `variable_details`) for project teams, including those who are not R users.
...
- **Plan for future needs**: Align with existing R ecosystem packages and keep the possibility of supporting standards such as DDI and LinkML open.

## 1. Integration with the R ecosystem

The label system in `recodeflow` will use **existing standards** from packages like `haven` and `labelled`:

- For variable labels, we'll use `labelled::var_label()` to assign labels (e.g., `var_label(x) <- "Age of respondent"`) rather than directly manipulating attributes.

- For value labels, we'll leverage the `labelled::set_value_labels()` function to create `haven_labelled` class variables (e.g., `set_value_labels(x, "Censored" = 0, "Transplant" = 1, "Dead" = 2)`).

- Avoid duplicating existing functions for applying labels. Instead, we'll build on these established utilities and provide **wrapper functions** only if they add new `recodeflow`features like syncing metadata in`variables` (`variables.csv`) or `variable_details` (`variable_details.csv`.
...
# NOT RECOMMENDED: Base R approach
# attr(data$status, "label") <- "Patient status"
# attr(data$status, "labels") <- c("Censored" = 0, "Transplant" = 1, "Dead" = 2)

# RECOMMENDED: Using `haven` package
library(haven)
data$status <- haven::labelled(
  data$status,
  labels = c("Censored" =.0, "Transplant" = 1, "Dead" = 2),
  label = "Patient status"
)

# ALTERNATIVELY: Using `labelled` package
library(labelled)
data <- labelled::set_variable_labels(data, status = "Patient status")
...
Both sets of labels will be:

- Stored in the metadata files (variables.csv and variable_details.csv), which act as the single source of truth for all metadata.

- Applied as R attributes to the data frame so that labelling remains intact for analysis or export.
...
- Label attributes are lightweight (often just a single string or a small named vector).

- Bulk operations (e.g., setting labels on an entire dataset) will help avoid repeated overhead.

Import/export:

- CSV-first: For day-to-day collaboration, CSV remains the easiest way to share metadata with non-R stakeholders.

- Additional formats: DDI (XML) or LinkML (YAML/JSON) could be supported in future expansions, letting advanced users integrate with metadata repositories or codebook standards.

- SPSS/Stata: By adhering to `haven_labelled`, `recodeflow` automatically preserves labels in.sav and.dta exports.

## 5. Relationship to tagged NA functionality

`recodeflow` will accommodate `haven`::tagged_na() to represent multiple missing data categories (e.g., NA(a) for "not applicable"). Labels can be assigned to these tagged values just as they would be to any other numeric code. A separate tagged-NA specification will detail how to classify different missingness types in the metadata (e.g., 99 = "Refused") and convert them automatically into tagged_na('x') in R. Labels help clarify the meaning of each missing code in data dictionaries or codebooks.

## 6. Versioning and extensibility

Versioning: • Use GitHub releases to manage changes to label functionality. • Encourage storing metadata files under version control so label history can be diffed.

Extensibility:

- Additional fields (e.g., short/long labels, notes, or variable roles) can be appended in the metadata files (variables.csv and variable_details.csv) without requiring changes to the data frame attributes.

- Only essential attributes (short label, value labels) will be stored in R to avoid bloat or attribute loss during transformations.

## 7. Conclusion

By standardizing how labels are assigned, stored, and preserved, `recodeflow` will streamline metadata management for data harmonization projects. Integrating with `haven` and `labelled` ensures minimal redundancy, while CSV-based metadata files guarantee accessibility for non-R collaborators. Future steps can include DDI or LinkML export, enhanced versioning, and more robust transformation tracking. In the meantime, these core specifications will provide a solid foundation for a clear, dependable label management system.
...
- For variable labels, we'll build on the conventions established by these packages, where functions like `labelled::var_label()` assign labels (e.g., `var_label(x) <- "Age of respondent"`).

- For value labels, we'll leverage the approach used by functions like `labelled::set_value_labels()` to create `haven_labelled` class variables (e.g., `set_value_labels(x, "Censored" = 0, "Transplant" = 1, "Dead" = 2)`).

- Rather than directly using these functions, `recodeflow` will provide **wrapper functions** that follow the same conventions but add functionality specific to our workflow. These wrappers will enhance the base functionality by integrating with the metadata stored in `variables.csv` and `variable_details.csv`.
...
By keeping the authoritative labels in the metadata files, we avoid divergence between the data object and its documented metadata. If labels are updated via R functions, `recodeflow` will also update the metadata files (or alert the user to do so) to maintain consistency. Conversely, if the CSV metadata is changed (e.g., a team member updates a label in variables.csv outside of R), re-importing or re-applying the metadata will update the R object's labels accordingly. This design balances flexibility with consistency.
...
In summary, `recodeflow` will treat variable labels and value labels as first-class metadata, storing them in both the metadata files and as attributes. Clear APIs for setting, getting, and modifying these labels ensure efficiency and reduce manual effort. Proper labeling of categorical data will be enforced so that any factor or coded variable has an accompanying label set, improving the interpretability of harmonized datasets.

## 3. Supporting a single source of truth for metadata

A fundamental principle of `recodeflow` is maintaining a single authoritative source for metadata to ensure consistency across team members and throughout the analysis workflow:

- **Central metadata repository**: The `variables.csv` and `variable_details.csv` files serve as the definitive source of truth for all metadata, including variable and value labels. This approach ensures that all collaborators, including those who don't use R, can access and update metadata.

- **Bidirectional synchronization capabilities**: The system will provide capabilities for:

    - Retrieving variable and value labels from the metadata files for use in R
    - Applying these labels to data frames using R attributes
    - Optionally updating the metadata files when labels are modified in R

- **Selective application**: Rather than automatically applying labels during every data transformation, the design will allow users to explicitly manage when and how labels are applied, maintaining a clear separation between data processing and metadata management.

- **Visualization and reporting integration**: The metadata from the central repository will be accessible for use in creating tables (with packages like `gt` or `kableExtra`) and visualizations (with `ggplot2`), ensuring consistent labeling throughout the research pipeline from data processing to final outputs.

This approach strikes a balance between maintaining authoritative metadata and providing flexibility in how and when that metadata is applied to data objects in R.
...
**Memory efficiency:** Storing labels as attributes (which are essentially small vectors or strings attached to each column) is generally lightweight. A variable label is just a single string, and value labels are a vector of label mappings which, in most cases, is much smaller than the data itself (since it only has one entry per unique code). We will ensure that these attributes are only as large as necessary and avoid duplicating data. For example, if a variable has 100 possible values but only 5 are actually labeled, we will only store those 5 label pairs, not all 100 values. This is consistent with how `haven_labelled` works, where labels need not be exhaustive (Wickham, 2021).

**Avoiding data copies:** In R, adding or modifying attributes to a vector can trigger a copy of that vector in memory (Wickham, 2019, p. 138-141). To mitigate this, `recodeflow` may consider implementing efficient label operations, such as applying variable labels to all data frame columns in a single function call rather than iterating over columns individually. This approach leverages vectorized operations, which are more memory-efficient in R as demonstrated by Mount & Zumel (2019) in their work on efficient data manipulation.

**Efficient lookup and manipulation:** Functions to get or set labels will be optimized to avoid unnecessary data scanning. Retrieving a label should be near O(1) (constant time) since it can directly fetch the attribute or look up in a keyed sidecar table, consistent with established lookup complexity principles (Cormen et al., 2009). Setting a label will typically be O(1) for a variable label or O(m) for m labels in a value labels vector. We will also consider using `data.table` or keyed `tibbles` for the metadata files to allow fast joins or lookups by variable name, leveraging the efficient indexing mechanisms described by Dowle & Srinivasan (2019) in the `data.table` package documentation.
...
## 5. Export and import compatibility
...
- **Short and long variable labels:** Many datasets distinguish between a short label (a brief name) and a longer description. For example, a variable BMI might have a short label "Body Mass Index" and a long description "BMI, calculated as weight (kg) / height (m)\^2". In `recodeflow`, we plan to support storing both. The variables sheet can have separate columns, e.g., label_short and label_long (or description). The short label would be what we treat as the "label" attribute in R (concise enough to show in a data viewer), while the long label/description would be stored only in the metadata files (and used for documentation generation like codebooks or when someone needs more context). This aligns with practices in codebook standards and IPUMS-like metadata, where variable labels and descriptions are distinct fields. We will ensure the functions allow access to both (e.g., **get_variable_description(name)** might retrieve the long description). However, to keep the R object lightweight, we will not attach the full long description as an attribute to every vector (which could be quite large text). Instead, the long descriptions live in `variable_details` or variables files and can be queried on demand.

- **Additional fields (notes, sources, etc.):** The variable_details file already supports a Notes column (used to convey special instructions or cautions about the variable). We will continue to support this and similar fields. Notes are typically free-form text explaining quirks in recoding or definitions. Other metadata we may consider: data source, survey question text, units of measurement, allowed ranges, etc., depending on user needs. Our principle is to house this information in the metadata files rather than in the main data structure. This is because the metadata files can be easily extended with new columns without affecting how the data frame operates in R. If we were to add every piece of metadata as an attribute to the data frame, it would become cumbersome to manage. It might bloat the object (and many R functions would ignore or drop those attributes anyway). Using the metadata files approach, we can add as many fields as needed (within reason) to variables.csv/variable_details.csv. For example, adding a units column for variables that have units (like "kg" for weight) could be done in the CSV, and `recodeflow` can have a function get_units(var) to retrieve it.

- **Integration with visualization and reporting:** The metadata stored in the central repository will be accessible for creating tables and visualizations, ensuring that variable and value labels are consistently used throughout the analysis workflow. By providing helper functions that access the metadata repository, users can automatically use the correct labels in plots (e.g., axis titles in ggplot2) and tables (e.g., variable names and value labels in gt tables) without manually copying or redefining them. This maintains the single source of truth principle and ensures that when labels are updated in the metadata files, all outputs automatically reflect these changes.

- **Maintaining compatibility with metadata structure:** We will not create a complex S3 class to hold all metadata in R at this time. Instead, the metadata files (likely tibbles read from CSV) will remain the primary carriers of extensive metadata. This decision is due to resource constraints and also to maintain clarity – the `recodeflow` workflow already expects these files, so expanding them is natural. We will, however, provide convenient R accessors. Perhaps we will have an internal list or environment where the metadata tables are stored when loaded (for quick lookup), and methods to query them. But the raw storage stays as two data frames with defined columns. This also makes it easy to export the entire metadata in one go (since it's basically those two tables).

- **Functions for setting/getting metadata:** To manage these metadata fields, we'll implement functions akin to:

    - `set_variable_metadata(var, short_label=NULL, long_label=NULL, description=NULL, notes=NULL, ...)` that can take one or more metadata pieces and assign them (updating the metadata files and any relevant attribute, like the short label to the data).
    ...
    - Specific helpers like `set_notes(var, text)` or `get_notes(var)` for the Notes field, to encourage documentation of variables within R. These functions will abstract the underlying data structure (CSV or tibble), giving the user a clear API. Internally, they manipulate the variables/variable_details files and possibly attributes. By doing this, we ensure efficiency (directly editing a data frame in memory and then writing out to CSV when needed) and reduce user error (it's safer than manually editing the files without guidance).

- **Ensuring minimal coupling to data object:** Because we are not embedding all metadata in the data frame, we avoid issues with large attributes. The only attribute attached by default will be the short variable label and the value labels. Everything else (notes, long descriptions, etc.) stays in the metadata files. This keeps the data frame S3 object clean and focused, which is important given that some R functions drop unknown attributes. We will document that the full metadata is always available via `recodeflow`'s metadata files and functions, even if not attached to the data frame.

In summary, the label functionality is built with extensibility in mind: today it's labels, tomorrow it could be definitions, provenance, etc. The design relies on the flexible metadata files approach, avoiding heavy modifications to the core data object. This allows `recodeflow` to grow in metadata scope without large refactoring. We will maintain backward compatibility with projects that might not use new fields (extra columns can be optional), and ensure that any additional metadata fields do not break existing functions. By doing so, we keep `recodeflow` lean yet ready to capture rich data documentation as needed.
...
## 9. Extensibility for additional metadata

While the primary focus is on labels, the design will allow extensibility to other metadata fields such as short vs. long labels, descriptions, and notes. We will incorporate these to complement the existing variables and variable_details structure, rather than overloading the R objects with extra attributes.

- **Short and long variable labels:** Many datasets distinguish between a short label (a brief name) and a longer description. For example, a variable BMI might have a short label "Body Mass Index" and a long description "BMI, calculated as weight (kg)/height (m)\^2". In `recodeflow`, we plan to support storing both. The variables sheet can have separate columns, e.g., label_short and label_long (or description). The short label would be what we treat as the "label" attribute in R (concise enough to show in a data viewer), while the long label/description would be stored only in the metadata files (and used for documentation generation like codebooks or when someone needs more context). We will ensure the functions allow access to both (e.g., **get_variable_description(name)** might retrieve the long description). However, to keep the R object lightweight, we will not attach the full long description as an attribute to every vector (which could be quite large text). Instead, the long descriptions live in `variable_details` or variables files and can be queried on demand.

- **Additional fields (notes, sources, etc.):** The variable_details file already supports a Notes column (used to convey special instructions or cautions about the variable). We will continue to support this and similar fields. Notes are typically free-form text explaining quirks in recoding or definitions. Other metadata we may consider: data source, survey question text, units of measurement, allowed ranges, etc., depending on user needs. Our principle is to house this information in the metadata files rather than in the main data structure. 

There may be exceptions in the future, such as `units` which is currently supported in `hmisc` and could have often used in tables and figures.
...
## 10. Technical documentation and usability
...
## 11. Potential challenges and open questions
...
11. **haven: Import and export 'SPSS', 'Stata' and 'SAS' files**

    The `haven` package documentation provides details on how labels are handled when importing and exporting data between R and other statistical software formats.

    *Link:* <https://CRAN.R-project.org/package=haven>

12. **Advanced R**

    This book provides in-depth coverage of R's memory management and how operations like attribute modification can trigger copying of data objects in memory.

    *Link:* <https://adv-r.hadley.nz/>

13. **Efficent data manipulation for labels\
    **
    ...
    Mount, J., & Zumel, N. (2019). Practical Data Science with R (2nd ed.). Manning Publications.
