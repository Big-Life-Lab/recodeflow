...
\`\`\`r result \<- standardize_age(age_vector, units_vector)

recodeflow/ ├── R/ │ ├── core/ \# Core class definitions and methods │ ├── metadata/ \# Metadata management functions │ ├── standards/ \# DDI, PMML, Dublin Core integration │ ├── recode/ \# Recoding functionality │ └── utils/ \# Helper functions ├── tests/ │ └── testthat/ \# Unit tests ├── vignettes/ │ ├── getting-started.Rmd │ ├── metadata-guide.Rmd │ └── etc.Rmd ├── data/ \# Example datasets ├── docs/ \# Project documentation │ └── specifications/ \# Technical specifications └── inst/ └── extdata/ \# Meeting notes and decisions